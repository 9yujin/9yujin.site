...
this와 this가 가리킬 객체를 연결하는 것을 this 바인딩이라고 부른다. 함수 호출방식에 따라 다르게 바인딩 된다.
...
중첩함수와 콜백함수로 일반함수를 선언했기 때문에 전역객체가 바인딩 된다. 만약에 중첩이나 콜백함수를 이용해서 this를 사용해서 자신이 속한 객체의 메서드에 접근하려고 한 상황에서는 의도와 다르게 바인딩이 된 것이라고 할 수 있다.
...
함수를 객체의 메서드로 호출할 경우, this는 메서드를 호출한 객체, 즉 점\(.) 바로 앞에 적힌 객체로 바인딩 된다.

여기서 this는 inner를 가리킨다.
...
`function.prototype`의 메서드로서, 모든 함수가 상속받아 사용할 수 있다.
...
일반적으로 내부에서 this 호출 시, 클래스가 생성한 인스턴스 객체가 바인딩된다. 그러나 클래스 내의 모든 코드에는 암묵적으로 **strict mode**가 적용된다. 만약 메서드 내부에 중첩이나 콜백 함수를 일반 함수로 선언했을때, 엄격모드니까 undefined가 바인딩된다.
...
[참고](https://www.youtube.com/watch?v=DTX52Pv7PZM)
