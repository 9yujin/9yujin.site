...
```tsx filename="createChallengeSlice.ts"
...
- `onSuccess`와 `onError`를 통해 요청이 성공했을 때와 실패했을 때 실행할 로직을 넣어줄 수 있다. ` defaultOptions` 에서 공통으로 에러핸들링을 해준다면, `useQuery`를 사용하는 컴포넌트에서는 오직 요청이 성공했을 때만의 상황만 볼 수 있다.
